...
function balanceOf(address owner) external returns (uint256); function transfer(address to, uint256 amount) external returns (bool); function decimals() external returns (uint256); }
...
uint public RTC_BLOCKS_PER_EPOCH_TARGET = 5 ;
uint public RTC_BLOCK_TIME = 600 ; 
uint public ETHER_BLOCK_TIME = 15 ; 
uint public halvingBlockAmount = 25000 ; 

uint public ETHER_BLOCKS_PER_EPOCH_TARGET = (RTC_BLOCK_TIME.div(ETHER_BLOCK_TIME)).mul(RTC_BLOCKS_PER_EPOCH_TARGET) ;
...
function newBlockChallenge() internal {
    ...
    if (blockNumber % RTC_BLOCKS_PER_EPOCH_TARGET == 0) {
        adjustDifficulty() ; 
    }
...
function testHASH(uint256 nonce, bytes32 challenge_digest) public view returns (bool success) { bytes32 digest = keccak256(challenge, msg.sender, nonce);
...
library Limit { function limitLessThan(uint a, uint b) internal pure returns (uint c) {
